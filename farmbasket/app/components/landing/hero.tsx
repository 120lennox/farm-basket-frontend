export default function Hero() {
  return (
    <div>
      <div className="mt-20">
        <div>
          <div>
            <div>
              <div>
                <div>
                  <h1 className="text-[96px] font-semibold">
                    Farm Basket
                  </h1>
                </div>
                <div className="w-[950px]">
                  <p className="text-[32px]">
                    From Seeds to Harvest: Your One-Stop Shop for All Things Agriculture
                    Seeds to Harvest: Your One-Stop Shop for All Things Agriculture Your
                    Harvest: Your One-Stop Shop for All Things Agriculture
                  </p>
                </div>
              </div>
              <div className="mt-8">
                <button className="bg-gray-400 p-4 px-10 rounded-2xl font-normal">
                  Get Started
                </button>
              </div>
            </div>
            <div className="mt-10 mb-5">
              <div className="text-[48px] font-semibold">
                What's Trending?
              </div>
            </div>
          </div>
          <div className="mt-8">
            <button className="bg-gray-400 p-4 px-10 rounded-2xl font-normal">
              Get Started
            </button>
          </div>
        </div>
<<<<<<<<< Temporary merge branch 1
        <div className="mt-10 mb-5">
          <div className="text-[48px] font-semibold">What's Trending?</div>
        </div>
        <div className="flex justify-between items-center mt-14">
          <div className="bg-gray-400 h-80 w-72 rounded-md"></div>
          <div className="bg-gray-400 h-80 w-72 rounded-md"></div>
          <div className="bg-gray-400 h-80 w-72 rounded-md"></div>
        </div>
        <div className="mt-10 flex justify-center items-center flex-row space-x-2">
          <div className="bg-gray-400 rounded-full h-4 w-4"></div>
          <div className="bg-gray-400 rounded-full h-4 w-4"></div>
          <div className="bg-gray-400 rounded-full h-4 w-4"></div>
        </div>
      </div>
    </div>
=========
>>>>>>>>> Temporary merge branch 2
  );
}
