...
// Stock context for managing products
...
const StockProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const updateProduct = (updatedProduct: Product) => {
    setStock((prevStock) =>
      prevStock.map((product) => (product.id === updatedProduct.id ? updatedProduct : product))
    );
  };

  const deleteProduct = (productId: string) => {
    setStock((prevStock) => prevStock.filter((product) => product.id !== productId));
  };
  ...
  return (
    <StockContext.Provider value={{ stock, updateProduct, deleteProduct, addProduct }}>
      {children}
...
// StockPage Component
const StockPage = () => {
  const { stock, updateProduct, deleteProduct, addProduct } = useStock();
